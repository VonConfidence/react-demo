  ...
  render() {
    ...
    const usercollectionList = usercollection.length
      ? usercollection.map((ucItem, index) => (
        ...
        <Card key={index} title={ucItem.title} extra={<a href = {`/#/details/${ucItem.uniquekey}?title=${ucItem.title}`}>查看</a>}>
          <p>{ucItem.title}</p>
        </Card>
      ))
      : '你还没有收藏任何文章'
      ...
      usercomment.map((comment,index)=> (
        <Card key={index} title={`于 ${comment.datetime} 评论了文章  ${comment.title}`} extra={<a href={`/#/details/${comment.uniquekey}?title=${comment.title}`}>查看</a>} >
